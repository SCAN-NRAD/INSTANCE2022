...
def unet_with_groups(input: jnp.ndarray, zooms: Tuple[float, float, float]) -> jnp.ndarray:
    ...
    """
    kw = dict(irreps_sh=e3nn.Irreps("0e + 1o + 2e"), num_radial_basis=2)
    ...
    x = conv(f"{round(mul)}x0e + {round(mul)}x1o", x, radius=r)
    x_a = x = cbg(
        x,
        mul,
        filter=["0e", "0o", "1e", "1o"],
